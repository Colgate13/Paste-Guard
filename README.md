...
A browser extension that displays a confirmation before pasting content on selected websites, allowing you to have control over the paste action. Compatible with Google Chrome and Mozilla Firefox using `manifest_version: 3`.
...
This extension intercepts the paste (`paste`) event on selected websites and displays a confirmation to the user. If the user confirms, the paste proceeds normally. Otherwise, the paste is canceled. You can manage which sites have paste protection enabled via the extension's popup interface.
...
1. **When attempting to paste content** on a website (using `Ctrl + V` or right-clicking and selecting "Paste"), the extension checks if the site is listed as protected.
2. **If the site is protected**, a dialog box will appear asking: **"Do you want to paste the clipboard content?"**
   - **If you click "OK"**, the content will be pasted normally.
   - **If you click "Cancel"**, the paste will be canceled.
3. **If the site is not protected**, pasting will proceed without confirmation.

You can enable or disable paste protection for specific websites using the extension's popup interface.
...
- **Permissions:**
  - The extension requires `storage` permission to save your site preferences and `activeTab` permission to interact with the current website.

---

## Privacy Policies

**Privacy Policy for the "Paste Guard" Extension**

**1. Information Collection**
- **Personal Information:** The extension **does not** collect, store, or transmit any personally identifiable information.
- **Usage Data:** The extension stores a list of protected sites ("protectedSites") locally in your browser. This data is used solely to customize the extension's behavior based on your preferences.
...
- The information stored is used exclusively to:
  ...
  - Manage the list of websites where paste protection is enabled.
